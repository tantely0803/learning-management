...
export const listTransactions = async (
  ...
  try {
    ...
    res.json({
      message: "Transactions retrieved successully",
      data: transactions,
    });
  } catch (error) {
...
export const createTransaction = async (
  ...
  try {
    ...
    // 3 create inital course progress
    const initialProgress = new UserCourseProgress({
      userId,
      courseId,
      enrollmentDate: new Date().toISOString(),
      overallProgress: 0,
      section: course.sections.map((section: any) => ({
    ...
    // add enrollement to relevant course
    await Course.update(
      { courseId },
      {
        $ADD: {
    ...
    res.json({
      ...
      data: {
        ...
        coureProgress: initialProgress,
      },
    });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error creating transation and enrollement", error });
  }
};
